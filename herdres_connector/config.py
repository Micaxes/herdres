...
def pinned_account_enabled(env: Any | None = None) -> bool:
    ...
    credential files/Keychain — named metadata fields only, never tokens — plus the
    remaining 5h/weekly quota). Default on; degrades to no line when the credentials are
...
def usage_refresh_seconds(env: Any | None = None) -> int:
    """Disk-cache TTL for the usage snapshot behind the pinned account line. Coarse on
