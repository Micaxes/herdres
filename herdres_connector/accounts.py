...
# macOS stores the Claude credentials in the login Keychain instead of the file above.
...
def _read_claude_credentials() -> dict[str, Any]:
    """The Claude OAuth credentials as a dict. Linux writes ~/.claude/.credentials.json;
    macOS keeps them in the login Keychain (service 'Claude Code-credentials'), so when the
    file is absent fall back to a read-only `security` lookup. The token is read in-process
    and never rendered or logged."""
    data = _read_json(CLAUDE_CREDENTIALS_PATH)
    if data:
        return data
    if sys.platform == "darwin":
        try:
            result = subprocess.run(
                ...
                capture_output=True, text=True, timeout=HTTP_TIMEOUT_SECONDS, check=False,
            )
...
def claude_identity(path: Any = None) -> dict[str, str]:
    creds = _read_json(path) if path else _read_claude_credentials()
...
def _claude_limits() -> dict[str, Any]:
    ...
    oauth = _read_claude_credentials().get("claudeAiOauth")
